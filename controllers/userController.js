        ...
        if (flag == 0) {
            ...
            if (couponCode.availability == "common") {
                await userModel.findByIdAndUpdate(
                    ...
                    { $addToSet: { coupons: couponCode.couponCode } }
                );
...
const createNeworder = async function (req, res) {
    try {
        ...
        if (req.body.usedCoupon) {
            ...
            const upUserCoupon = await userModel.findByIdAndUpdate(
                { _id: req.session.user },
                { $pop: { coupons: -1 } }
            );
        }
        ...
        const instance = new Razorpay({
            key_id: "rzp_test_qmjEny8LnAs8hb",
            key_secret: "0Bg7w3NJUvynTzMpA9QYzahK",
        });
...
const onPaymentSuccess = async function (req, res) {
    try {
        ...
        let downCount = await GamesModels.findById(
            { _id: gamId },
            { _id: 0, downloads: 1 }
        );
        ...
        await GamesModels.findByIdAndUpdate(
            { _id: gamId },
            { $set: { downloads: downCount } }
        );
        const price = orderData.total;
        await userModel.findByIdAndUpdate(
            { _id: usId },
            { $set: { myLastPrice: price } }
        );
        await userModel.findByIdAndUpdate(
            { _id: usId },
...
const loadCart = async function (req, res) {
    try {
        const userID = req.session.user;
        let cartItem = await userModel.findById(
            { _id: userID },
            { cartItems: 1, _id: 0 }
        );
...
const creatingCartItem = async function (req, res) {
    try {
        ...
        if (flag == 0) {
            const cartUp = await userModel.updateOne(
                { _id: id },
                {
                    $addToSet: {
                        cartItems: {
...
const deleteCartItem = async function (req, res) {
    try {
        ...
        userCart.splice(i, 1);
        const newUpUser = await userModel.findByIdAndUpdate(
            { _id: userID },
            { $set: { cartItems: userCart } }
        );
...
const checkingTheCouponValidity = async function (req, res) {
    try {
        const userCoupon = await userModel.findById(
            { _id: req.session.user },
            { _id: 0, coupons: 1 }
        );
...
const findGames = async function (req, res) {
    try {
        ...
        gameCollection = await GamesModels.find({
